...
{
  // std::cerr << "enter RoutingTable lookup" << std::endl;
  // FILL THIS IN
  // ip和mask按位与，匹配dest
  // 在 m_entries 中使用最长前缀匹配

  RoutingTableEntry result;
  bool found = false;
  if(!m_entries.empty()){
    // std::list<RoutingTableEntry>::const_iterator tableItem;
    for(auto t = m_entries.begin(); t != m_entries.end(); ++t){
      uint32_t cmp = ip & t->mask;
      uint32_t table_addr = t->dest & t->mask;
      if(cmp == table_addr){ 
        if(!found || result.mask <= t -> mask){ 
          result = *t;
          found = true;
        }
      }
