...
public:
  ...
  bool isCorrectMac(const uint8_t* mac, const Interface& inputIface);
  void handleIPV4(Buffer &packet, const Interface* iface, uint8_t* src);
  void send_t3_packet(Buffer &packet, const Interface* iface, uint8_t type, uint8_t code);
  void handleARP(uint8_t* arp_packet, const Interface* iface, uint8_t* src);
